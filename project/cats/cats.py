...
def about(subject):
    ...
    def f(para):
        ...
        p=""
        for c in para:
            p+="" if c in string.punctuation else c
        p=" "+p+" "
        print("DEBUG:", p)
        for s in subject:
...
def accuracy(typed, source):
    ...
    if (len(typed_words)==0):
        return 0.0 if len(source_words) else 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if (diff<min):
            s=word
            min=diff
    return s if min<=limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    def f(s1,s2,now,limit):
        if (s1=="" or s2==""):
            return now+abs(len(s1)-len(s2))
        if (now>limit):
            return now
        return f(s1[1:],s2[1:],now+(s1[0]!=s2[0]),limit)
    return f(typed,source,0,limit)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0]==source[0]: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        t=minimum_mewtations(typed[1:],source[1:],limit)
        return t if t<=limit else limit+1
        # END
    else:
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamp in timestamps_per_player:
        ...
        for i in range(len(timestamp)-1):
            t.append(timestamp[i+1]-timestamp[i])
        times.append(t)
    return words,times
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        min=get_time(match_object,0,i)
        id=0
        for j in player_indices:
            time=get_time(match_object,j,i)
            if (time<min):
                min=time
                id=j
        words[id].append(get_word(match_object,i))
    return words
    # END PROBLEM 11
