...
class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            s="increased" if self.before<self.after else "decreased"
            return f"{self.id}: {s} {self.before}->{self.after}"
        return str(self.id) + ': ' + msg

class BankAccount:
    """A bank account that traks its transaction history.c
...
def make_change(amount, coins):
    ...
    if amount < smallest:
        return None
    "*** YOUR CODE HERE ***"
    t=[] if amount == smallest else make_change(amount-smallest,rest)
    if t!=None:
        return [smallest]+t
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        copy = dict(self.coins)
        for i in change:
            count = copy.pop(i) - 1  # The coin denomination is removed
            if count:
                copy[i] = count
        self.coins=copy
        print("DEBUG:",copy)
